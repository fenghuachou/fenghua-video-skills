...
if [ "$#" -lt 3 ] || [ "$#" -gt 4 ]; then
  echo "Usage: $0 <input_master.mp4> <background.png> <output.mp4> [video_width]" >&2
  exit 1
fi
...
ffmpeg -y \
  ...
  -i "$INPUT_MASTER" \
  -filter_complex "[1:v]scale=${VIDEO_WIDTH}:-2[fg];[0:v]fps=${FPS}[bg];[bg][fg]overlay=0:(H-h)/2:shortest=1,format=yuv420p[v]" \
  -map "[v]" -map 1:a? \
